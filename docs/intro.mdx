...
Flags:
      --ami-license-file string   unique AMI add-on license file path
  -c, --config string             application config file
  -u, --disable-db-update         disable updating scanner DB automatically
  -h, --help                      help for ec2-mac-ami-scan
  -i, --ignore-system-volume      do not scan the system volume
  -l, --license-file string       license file path (default "scanner.lic")
      --min-score float32         do not show vulnerabilities with lesser score
      --min-severity string       do not show vulnerabilities with lesser severity
...
**Currently, we only provide a scanner binary for Amazon Linux x86_64. It is archived into a tar.gz and available on our site or at https://downloads.veertu.com/#ec2-mac-ami-scan/.**

:::

### Preparing an EC2 instance

- Create an x86_64 **Amazon Linux** EC2 instance with a minimum of 1 CPU, 0.5GiB of RAM, and ~1GB of space for storage of the vulnerability database.
...
1. [Download the latest Linux x86_64 package.](https://veertu.com/downloads/ec2-mac-ami-scan-linux/)
    ```bash
...
You need two different licenses to scan an AMI.

1. The "Platform" license for the specific host to use the binary.
2. The "AMI Add-On" license for the specific AMI[s] you want to scan.

Here is the step by step:

1. Set up the AWS CLI and make sure the ~/.aws/credentials file is set up with the correct credentials for the root user.
2. Obtain licenses: [Trial](https://veertu.com/ec2-mac-ami-scan-trial/)  - or email support@veertu.com to get a non-trial license
3. Execute the license CLI to activate it:
    ...
    ```bash
    sh-4.2$ ./ec2-mac-ami-scan license activate --help
    Activate the license

    Usage:
      ec2-mac-ami-scan license activate {activation-key} [flags]

    Flags:
          --ami-count   activate Unique AMI add-on license
      -h, --help        help for activate
      -o, --overwrite   overwrite license file if already exists
    ...
    ```bash
    sh-4.2$ ./ec2-mac-ami-scan ami-XXXXXXXXXXXXXXXXX
    Error: unique AMI Scan license is required for this scan

    sh-4.2$ ./ec2-mac-ami-scan license activate --ami-count XXXX-XXXX-XXXX-XXXX
    Activated
    ```
...
- Why do I need two licenses?
    - The "Platform" license is for the specific host to use the binary.
    - The "AMI Add-On" license is for the specific AMI[s] you want to scan.
- Why do I need to set up the AWS CLI?
    - The AWS CLI is used to get the AMI snapshot and mount it as a volume.
- For the AMI Add-On license, can I scan multiple AMIs with the same license?
    - Yes, you can scan multiple AMIs with the same license as long as you've paid for the amount of AMIs you want to scan.

## Release Notes

### 2.0.0 - Apr 24th, 2026

- Complete rewrite of the scanner for a new license structure.
